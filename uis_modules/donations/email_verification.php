...
if(isset($_REQUEST['code']) && isset($_REQUEST['rec_id'])){
	...
	if($arr['status'] == 'PENDING'){
		exec_query("UPDATE ".$GLOBALS['MOD_S_TABLES']['registration']." SET status='ACCEPTED' WHERE verification_code='".$_REQUEST['code']."'",Q_RET_NONE);
		if(is_query_ok() && get_affected_rows() > 0){
			...
			exec_query("DELETE FROM ".$GLOBALS['MOD_S_TABLES']['registration']." WHERE rec_id='".$_REQUEST['rec_id']."'",Q_RET_NONE);
