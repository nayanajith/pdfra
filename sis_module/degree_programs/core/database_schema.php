...
$program_table_schemas['marks']="CREATE TABLE `%smarks` (
  ...
  `degree_grade`     varchar(2)    DEFAULT null,
...
$program_table_schemas['transcript']="CREATE TABLE `%stranscript` (
  `id`                 INT NOT NULL AUTO_INCREMENT COMMENT 'Primary Key: Unique auto increment ID.',
  `user_id`            INT NOT NULL COMMENT 'User id from the users table to keep track of user',
  `index_no`           VARCHAR(8) NOT NULL COMMENT 'Students index number',
  `transcript_id`      CHAR(2) DEFAULT NULL COMMENT 'Transcript ID generated by the system',
  `info`               TEXT COMMENT 'Transcript information as a json object',
  `timestamp`          TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
   PRIMARY KEY (`id`),
   UNIQUE KEY (`transcript_id`),
   FOREIGN KEY (`user_id`) REFERENCES users(`user_id`) ON UPDATE CASCADE ON DELETE SET NULL
)ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='This table stores the infrormation of transaction generated by the users'";
