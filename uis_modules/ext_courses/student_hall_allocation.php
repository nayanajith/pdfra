...
function gen_exam_no($center_id,$hall_id,$student_id,$program_code){
	...
	$check=($check%$modulus);
	return $program_code.$exam_no.$check;
}

$query="SELECT DISTINCT(center),center_id FROM ".$GLOBALS['MOD_P_TABLES']['exam_hall'];
...
$first_run			=true;
$query="SELECT sutdents_allocated FROM ".$GLOBALS['MOD_P_TABLES']['exam_hall']." WHERE sutdents_allocated != 0";
...
foreach($exam_centers as $center => $center_id){
	...
	$query="SELECT * FROM ".$GLOBALS['MOD_P_TABLES']['exam_hall']." WHERE center_id = '$center_id' ORDER BY ABS(hall_id)";
	...
	*/
	...
	while($row=mysql_fetch_assoc($res)){
		...
		if($first_run){
			//$query="SELECT * FROM ".$GLOBALS['MOD_P_TABLES']['registration']." u, ".$GLOBALS['MOD_P_TABLES']['validation']." v WHERE u.registration_no=v.registration_no AND u.exam_center='$center' AND v.validation=1 ORDER BY ABS(u.lastname),ABS(u.firstname) LIMIT $affected_students,$students_for_hall";
			$query="SELECT * FROM ".$GLOBALS['MOD_P_TABLES']['registration']." u, ".$GLOBALS['MOD_P_TABLES']['validation']." v WHERE u.registration_no=v.registration_no AND v.validation=1 ORDER BY u.program,u.rec_id,ABS(u.last_name),ABS(u.first_name) LIMIT $affected_students,$students_for_hall";
		}else{
			//$query="SELECT * FROM ".$GLOBALS['MOD_P_TABLES']['registration']." u, ".$GLOBALS['MOD_P_TABLES']['validation']." v WHERE u.registration_no=v.registration_no AND u.exam_center='$center' AND v.validation=1 AND v.hall_allocated=0 ORDER BY ABS(u.lastname),ABS(u.firstname) LIMIT $affected_students,$students_for_hall";
			$query="SELECT * FROM ".$GLOBALS['MOD_P_TABLES']['registration']." u, ".$GLOBALS['MOD_P_TABLES']['validation']." v WHERE u.registration_no=v.registration_no AND v.validation=1 AND v.hall_allocated=0 ORDER BY u.program,u.rec_id,ABS(u.last_name),ABS(u.first_name) LIMIT $affected_students,$students_for_hall";
		}
		...
		while($student_row=mysql_fetch_assoc($students_res)){
			...
			$query_hall_allocated="SELECT hall_allocated FROM ".$GLOBALS['MOD_P_TABLES']['validation']." WHERE registration_no='".$student_row['registration_no']."'";
			...
			$query_alloc="INSERT INTO ".$GLOBALS['MOD_P_TABLES']['student_alloc']."(registration_no,exam_no,hall_id,room_no,center_id) values('".$student_row['registration_no']."','".gen_exam_no($center_id,$hall_id,$sequence,$program_code)."','".$hall_id."','".$room_no."','".$center_id."')";
			...
			if(get_affected_rows()>=1){
				...
				$query_hall_allocated="UPDATE ".$GLOBALS['MOD_P_TABLES']['validation']." SET hall_allocated=1 WHERE registration_no='".$student_row['registration_no']."'";
				$bla=exec_query($query_hall_allocated,Q_RET_MYSQL_RES);
				/*if insertion is ok setn new number of student allocated in each hall*/
				$set_allocated="UPDATE ".$GLOBALS['MOD_P_TABLES']['exam_hall']." SET sutdents_allocated='$sequence' WHERE hall_id='".$hall_id."' AND center_id='".$center_id."'";
...
<option value='all'>all</option>
		<?php 
			$query="SELECT distinct center_id,center FROM ".$GLOBALS['MOD_P_TABLES']['exam_hall'];
