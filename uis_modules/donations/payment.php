...
if(isset($_SESSION['username'])){
	$arr	=exec_query("SELECT * FROM ".$GLOBALS['MOD_S_TABLES']['registration']." WHERE email_1='".$_SESSION['username']."'",Q_RET_ARRAY);
	$_SESSION['user_id']=$arr[0]['rec_id'];
}

exec_query("UPDATE ".$GLOBALS['MOD_S_TABLES']['registration']." set registration_no='".gen_index_no($_SESSION['user_id'])."' WHERE rec_id='".strtoupper($_SESSION['user_id'])."'",Q_RET_NONE);
...
/*
echo " You can go back and change the information if required. <span style='color:red'>After you <b>logout</b> you will not be able to change them in next logins. Please <a href=\"javascript:open_page('registration_pg','application_pdf&data=true')\">download</a> and verify your application. </span> Click either pay offline to bank or pay online. The printed application and the payment invoice should be send in registered post to the following address: ";
*/
echo "<p> You can go back and change the information if required. <span style='color:red'>After you downloaded the application you will not be able to change the information again</span>. After verifying your information please <a href=\"javascript:open_page('registration_pg','application_pdf&data=true')\">download</a> your application. </span></p>
...
echo "<p>You can <a href=\"javascript:open_page('registration_pg','application_pdf&data=true')\">download</a> your application. </span></p>
<p>Click either pay offline to bank  if you want to pay the application processing fee offline or click pay online if you want to pay application processing fee using your credit card to.</p> 
<p>The printed application should be signed and send in registered post to the following address with the payment invoice: </p>";
}
...
if($reg_arr['status'] == 'ACCEPTED' || $reg_arr['status'] == 'ACCEPTED' ){
	...
	echo "<button dojoType='dijit.form.Button' type='submit' onClick=\"open_page('registration_pg','pay_offline')\">Pay offline to bank</button>";
	echo "<button dojoType='dijit.form.Button' type='submit' onClick=\"open_page('registration_pg','pay_online')\">Pay online&nbsp;&raquo;</button>";
}
echo "</div>";
