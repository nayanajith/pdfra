...
if(!isset($_REQUEST['receipt'])){
	$user_info=exec_query("SELECT status FROM ".$GLOBALS['MOD_P_TABLES']['registration']." WHERE rec_id='".strtoupper($_SESSION['user_id'])."'",Q_RET_ARRAY);
	...
	if($user_info['status'] == 'ACCEPTED'){
		...
		Please try again later or try our <a href=\"javascript:open_page('ext_courses','pay_offline')\">offline payment procedure</a>.
		";
	}
	return;
}
...
$test=exec_query("SELECT * FROM ".$GLOBALS['MOD_P_TABLES']['registration']." WHERE rec_id='".strtoupper($_SESSION['user_id'])."' AND registration_no='".$receipt['tp_ref_no'] ."'",Q_RET_ARRAY);
if(get_num_rows() != 1){
	session_destroy();
}


exec_query("UPDATE ".$GLOBALS['MOD_P_TABLES']['registration']." SET transaction_id='".$receipt['tr_ref_no']."'  WHERE rec_id='".strtoupper($_SESSION['user_id'])."'",Q_RET_MYSQL_RES);
switch($receipt['status']){
   ...
   case 'ACCEPTED':
		...
		exec_query("UPDATE ".$GLOBALS['MOD_P_TABLES']['registration']." SET status='ACCEPTED'  WHERE rec_id='".strtoupper($_SESSION['user_id'])."'",Q_RET_MYSQL_RES);
	...
	case'REJECTED':
		...
		exec_query("UPDATE ".$GLOBALS['MOD_P_TABLES']['registration']." SET status='REJECTED'  WHERE rec_id='".strtoupper($_SESSION['user_id'])."'",Q_RET_MYSQL_RES);
