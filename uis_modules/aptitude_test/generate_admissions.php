...
$query="SELECT index_no FROM bict_validation WHERE validation=1 AND admission_generated=0";
...
while($row=mysql_fetch_assoc($res)){
	...
	$admission=new Admission($row['index_no'],$date,$duration,$releaf,$issued_on);
	...
	$query="UPDATE bict_validation SET admission_generated=1 WHERE index_no='".$row['index_no']."'";
	$bla=mysql_query($query,$GLOBALS['CONNECTION']);
}
