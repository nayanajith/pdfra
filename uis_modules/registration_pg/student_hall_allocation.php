...
$query="SELECT DISTINCT(center),center_id FROM ".$GLOBALS['MOD_S_TABLES']['exam_hall'];
...
$first_run			=true;
$query="SELECT sutdents_allocated FROM ".$GLOBALS['MOD_S_TABLES']['exam_hall']." WHERE sutdents_allocated != 0";
...
foreach($exam_centers as $center => $center_id){
	...
	*/
	...
	while($row=mysql_fetch_assoc($res)){
		...
		if($first_run){
			//$query="SELECT * FROM ".$GLOBALS['MOD_S_TABLES']['registration']." u, ".$GLOBALS['MOD_S_TABLES']['validation']." v WHERE u.registration_no=v.registration_no AND u.exam_center='$center' AND v.validation=1 ORDER BY ABS(u.lastname),ABS(u.firstname) LIMIT $affected_students,$students_for_hall";
			$query="SELECT * FROM ".$GLOBALS['MOD_S_TABLES']['registration']." u, ".$GLOBALS['MOD_S_TABLES']['validation']." v WHERE u.registration_no=v.registration_no AND v.validation=1 ORDER BY u.program,u.rec_id,ABS(u.last_name),ABS(u.first_name) LIMIT $affected_students,$students_for_hall";
		}else{
			//$query="SELECT * FROM ".$GLOBALS['MOD_S_TABLES']['registration']." u, ".$GLOBALS['MOD_S_TABLES']['validation']." v WHERE u.registration_no=v.registration_no AND u.exam_center='$center' AND v.validation=1 AND v.hall_allocated=0 ORDER BY ABS(u.lastname),ABS(u.firstname) LIMIT $affected_students,$students_for_hall";
			$query="SELECT * FROM ".$GLOBALS['MOD_S_TABLES']['registration']." u, ".$GLOBALS['MOD_S_TABLES']['validation']." v WHERE u.registration_no=v.registration_no AND v.validation=1 AND v.hall_allocated=0 ORDER BY u.program,u.rec_id,ABS(u.last_name),ABS(u.first_name) LIMIT $affected_students,$students_for_hall";
		}
		...
		while($student_row=mysql_fetch_assoc($students_res)){
			...
			$query_hall_allocated="SELECT hall_allocated FROM ".$GLOBALS['MOD_S_TABLES']['validation']." WHERE registration_no='".$student_row['registration_no']."'";
			...
			if(get_affected_rows()>=1){
				...
				$query_hall_allocated="UPDATE ".$GLOBALS['MOD_S_TABLES']['validation']." SET hall_allocated=1 WHERE registration_no='".$student_row['registration_no']."'";
