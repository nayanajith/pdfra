...
class MCQ_paper {
   ...
   public function gen_stat($section){
      ...
      $markFile=$GLOBALS['MARK_FILE_STORE']."/".$this->self['paper_id'].".csv";
         ...
         foreach ($answers_array as $key => $answer) {
            ...
            if($answer != $this->blank){ //detect blank anwsers
               ...
               /*if(strlen($answer)>1 ){
                  ...
                  $multi_answer=preg_split($this->answer_delemiter,$answer);
               }*/
...
<table style='padding: 10px'>
   <tr>
      <td>Paper :</td>
      <td id='exam_id_td'><select name='paper_id' id='paper_id'>
      <?php
      openDB2("mcq_t");
      $SQL="SELECT * FROM mcq_paper";
      $RESULT=mysql_query($SQL,$GLOBALS['CONNECTION']);
      while( $ROW = mysql_fetch_array($RESULT) ) {
         echo "<option value='".$ROW['course_id']."-".$ROW['exam_id']."'>".$ROW['course_id']."&nbsp;@&nbsp;".$ROW['exam_id']."</option>";
      }
      closeDB();
      ?>
      </select></td>
