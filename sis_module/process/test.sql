...
BEGIN
   ...
   -- 4(ALL) compulsory course should be passed ('C') to get the HDIT  [ OBVIOUS ]
   ...
   -- count of compulsory courses sit by the student
   ...
   SELECT count(*) INTO comp_count_ FROM bit_marks INNER JOIN bit_course USING(course_id) WHERE index_no=index_no_ AND student_year=year_ AND repeat_max >= 1 AND compulsory AND grand_final_mark >= 50;
   ...
   -- RETURN CONCAT(lms_count_,opt_count_,comp_count_);

   IF lms_count_ >= to_sit_lms_min_ AND  opt_count_ >= to_sit_opt_min_ AND comp_count_ >= to_sit_comp_min_ THEN
      -- change the students stat into pass if the condition are fulfilled
      REPLACE INTO bit_student_state(`index_no`,`year2_state`,`year2_date`)values(index_no_,'HDIT',effective_date);
      RETURN CONCAT_WS(',','HDIT',effective_date);
   ELSE
      IF gpa >= min_gpa THEN
         -- If the student aquire the minimum GPA student will passw year 2
         REPLACE INTO bit_student_state(`index_no`,`year2_state`,`year2_date`)values(index_no_,'HDIT',effective_date);
      END IF;
      ...
      IF !lms_count_ AND year_ = 1 THEN
         SET to_sit_lms_ = ' TO SIT: LMS Assesments';
      ELSE
         SET to_sit_lms_ = ' ';
      END IF;

      IF comp_count_ = 0 THEN
         SET to_sit_comp_ = CONCAT('TO SIT: ALL Compulsory');
      ELSE
         SET to_sit_comp_ = CONCAT('TO SIT:',to_sit_comp_);
      END IF;

      IF opt_count_ = 0 THEN
         SET to_sit_opt_  = CONCAT(' TO SIT: ',to_sit_opt_min_,' FROM All Optional courses');
      END IF;

      IF opt_count_ != 0 AND to_sit_opt_min_ > opt_count_ THEN
         SET to_sit_opt_  = CONCAT(' TO SIT:',to_sit_opt_min_-opt_count_,' FROM (',to_sit_opt_,')');
      ELSE
         SET to_sit_opt_  = ' ';
      END IF;
      
      REPLACE INTO bit_student_state(`index_no`,`year2_state`)values(index_no_,CONCAT_WS(to_sit_comp_,to_sit_opt_,to_sit_lms_));
      RETURN CONCAT_WS(',',to_sit_comp_,to_sit_opt_,to_sit_lms_);
   END IF;
END;             
||
delimiter ;
