...
if(isset($_REQUEST['email']) && $_REQUEST['email'] != '' ){
	$user_arr=exec_query("SELECT * FROM ".$GLOBALS['MOD_P_TABLES']['employee']." WHERE email='".$_REQUEST['email']."'",Q_RET_ARRAY);
	$user_arr=$user_arr[0];
	if(get_num_rows() > 0 ){
		$password=rand(11111,22222);
		$password_md5=md5($password);
		$arr=exec_query("UPDATE ".$GLOBALS['MOD_P_TABLES']['employee']." SET password='".$password_md5."' WHERE email='".$_REQUEST['email']."'",Q_RET_MYSQL_RES);
		//echo $password;
		...
		if($mail->send_mail($GLOBALS['PAYMENT_ADMIN_MAIL'],$user_arr['email'],null,null,"Password reset for UCSC",$templates->reset_password_template($password))){
